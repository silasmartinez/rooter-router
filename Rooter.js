...
Rooter.prototype.add = function (routeUrl, routeResponse, routeMethod) {
  if (routeMethod) {
    ...
    for (var route in this.routeHandlers) {
...
Rooter.prototype._getBestMatch = function (reqUrlArray, reqMethod) {
  ...
  for (var route in this.routeHandlers) {
    ...
    if (reqUrlArray.length === parseInt(this.routeHandlers[route].hasLength, 10) && (this.routeHandlers[route].method === reqMethod || !this.routeHandlers[route].method)) {
...
Rooter.prototype._checkSplats = function (reqUrlArray) {
  var splatMatchArr = [];
  for (var splat in this.routeHandlers) {
    var matchSplat = true;
    var splatArr = splat.split('/');
